...
def pick_questions():
    problems = read_file("leetcode_questions_parsed.json")
    random_key = random.choice(list(problems.keys()))
    game.problems = problems[random_key]
    print(problems)
...
from questions import problems_dict
@app.route('/question', methods=['POST'])
def question():
    data = request.get_json()
    id = data['id']
    return make_response(True, {f"question{id}": problems_dict[id]})
